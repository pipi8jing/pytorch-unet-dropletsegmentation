...
p = imread('data/BTDFM-ROI1-8bit.tif');
p2 = imread('data/RL_DFM-ROI1-8bit.tif');

ptv = Img2Ddenoise_chu(p,0.2,20);
p2tv = Img2Ddenoise_chu(p2,0.2,20);

figure(23);clf;
subplot(1,2,1);imshow(p2);
subplot(1,2,2);imshow(uint8(p2tv));
...
% synthetic templates
t3 = 0.3+(0.7*strel('disk',15).Neighborhood)-0.7*padarray((strel('disk',12).Neighborhood),[3 3]);
z = strel('disk',15).Neighborhood-padarray((strel('disk',12).Neighborhood),[3 3]);
z(1:15,:) = 0;
t4 = t3-z;

figure(1);clf;
subplot(3,3,[1,4,7]);imshow(p2);
subplot(3,3,2);imshow(t1);
subplot(3,3,3);imshow(t2);
subplot(3,3,5);imshow(t3);
subplot(3,3,6);imshow(t4);
...
for ij = 1:length(ts)
    ...
    % local maxima of the correlation function
    fgm(ij,:,:) = imextendedmax(squeeze(cxfgm(ij,:,:)),.07,8);
    
    % identifying properties of the peaks
    ...
    % filtering the result based on correlation, intensity, area
    L2 = ismember(L,find(mC>0.3 & ar>3 & ar<100));
...
% choosing the final foreground markers as "the best" result or the sum of
% all results
