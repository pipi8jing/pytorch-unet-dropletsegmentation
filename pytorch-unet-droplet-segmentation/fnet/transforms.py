...
class Cropper(object):
    # 这里的n_max_pixels = n_img * width * height
    def __init__(self, cropping, by=16, offset='mid', n_max_pixels=16777216):
    ...
    def _calc_offsets_crop(self, shape_in, shape_crop):
        ...
        for i in range(len(shape_in)):
            ...
            if offset + shape_crop[i] > shape_in[i]:
                warnings.warn(
                    'Cannot crop outsize image dimensions ({}:{} for dim {}).'.format(offset, offset + shape_crop[i],
                                                                                      i))
